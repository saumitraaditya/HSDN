...
class BlockChainListener(threading.Thread):
    def __init__(self, name, exitFlag, eventTypeList):
    ...
    def run(self):
        while self.exitFlag:
            # print("\n Polling for events on BlockChain. \n")
            for event_type in self.eventTypeList:
                events = event_type.get_new_entries()
                for event in events:
                    print(event)
            time.sleep(5)
...
class SocialAgent(sleekxmpp.ClientXMPP):
    ...
    def __init__(self, user, pwd, server, controller=None, device_name=None):
        ...
        self.blockChainListener = BlockChainListener("BlockChainListener", self.blockChainFlag, self.blockChainEvents)
    ...
    def MSGListener(self, msg):
        ...
        if (setup == "resource_solicitation"):
            ...
            task_contract_address = task_description["task_contract_address"]
            ...
            for device_type in solicited_device_types:
                if (device_type in self.resources and len(self.resources[device_type])!=0):
                    ...
                    '''At this time in addition to doing the above I am going to deploy Network Delegation
                        contract and register it with the task contract.
                    '''
                    raw_contract_source = requests.get(task_description['contract']).text
                    compiled_contract = compile_source(raw_contract_source)
                    network_contract_interface = compiled_contract['<stdin>:' + "Networking_Delegation"]
                    w3 = Web3(Web3.HTTPProvider(task_description["blockchain_node"]))
                    network_contract = w3.eth.contract(abi=network_contract_interface['abi'],
                                                    bytecode=network_contract_interface['bin'])
                    # have to come up with a better method to handle accounts.
                    tx_hash = network_contract.constructor(self.xmpp_id).transact(
                        transaction={'from': w3.eth.accounts[0], 'gas': 1000000})
                    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
                    network_contract_address = tx_receipt.contractAddress
                    print("address of deployed network contract is {}".format(network_contract_address))
                    # create contract instance with deployed address
                    network_contract = w3.eth.contract(
                        address=network_contract_address,
                        abi=network_contract_interface['abi']
                    )
                    # register networking delegation
                    tx_hash = network_contract.functions.register_network(task_contract_address).transact(
                        transaction={'from': w3.eth.accounts[0]})
                    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
                    print("{} called register_network, gas used is {}".format(self.xmpp_id, tx_receipt.cumulativeGasUsed))
                    # whitelist task initiator so that he can install rules.
                    tx_hash = network_contract.functions.whiteList(task_contract_address).transact(
                        transaction={'from': w3.eth.accounts[0]})
                    tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
                    print("{} called whiteList for {}, gas used is ".format(self.xmpp_id, task_contract_address, tx_receipt.cumulativeGasUsed))
                    # now is also a good time to register event listeners.
                    dns_installations_event = network_contract.eventFilter('dns_path_installation',\
                                                       {'fromBlock': 0, 'toBlock': 'latest'})
                    self.blockChainEvents.append(dns_installations_event)
    ...
    ''' This method will read the task description file, fill in the relevant details,
        compile and deploy the contract and then
        send solicitation for the task by putting it in the broadcast queue
    '''
    def initiate_task(self, file_path):
        ...
        compiled_contract = compile_source(raw_contract_source)
        task_contract_interface = compiled_contract['<stdin>:'+ task.description["task"]]
        w3 = Web3(Web3.HTTPProvider(task.description["blockchain_node"]))
        task_contract = w3.eth.contract(abi=task_contract_interface['abi'], bytecode=task_contract_interface['bin'])
        # have to come up with a better method to handle accounts.
        tx_hash = task_contract.constructor().transact(transaction={'from': w3.eth.accounts[0], 'gas': 1000000})
        tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)
        ...
        print("address of deployed task contract is {}".format(task_contract_address))
